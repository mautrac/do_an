...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, temp_feat, cid, buffer_size=30):
        ...
        self.det_tlwh = np.asarray(tlwh, dtype=float)
...
class BYTETracker(object):
    def __init__(self, track_thresh, match_thresh, frame_rate=30, alpha_fuse = 0.9):
    ...
    def update(self, output_results, id_feature, cid, use_embedding=False):
        ...
        if len(strack_pool) > 0 and use_embedding: 
            track_features = np.asarray([track.smooth_feat for track in strack_pool], dtype=float)
        ...
        if use_embedding and sim_matrix is not None:
            ...
            dists = matching.fuse_embed_score(dists, sim_matrix, detections, alpha=self.alpha_fuse)
        else:
