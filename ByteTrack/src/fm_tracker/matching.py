...
def minarea_ious(atlwhs, btlwhs):
    ...
    :rtype ious np.ndarray
    """
    _minarea_iou_ = np.zeros((len(atlwhs),len(btlwhs)), dtype=float)
    if _minarea_iou_.size == 0:
        return _minarea_iou_

    atlwhs_np = np.ascontiguousarray(atlwhs, dtype=float)
    btlwhs_np = np.ascontiguousarray(btlwhs, dtype=float)
...
def dious(atlbrs, btlbrs):
    ...
    """
    _diou_ = np.zeros((len(atlbrs),len(btlbrs)), dtype=float)
    if _diou_.size == 0:
        return _diou_

    iou = ious(atlbrs, btlbrs)

    atlbrs_np = np.ascontiguousarray(atlbrs, dtype=float)
    btlbrs_np = np.ascontiguousarray(btlbrs, dtype=float)
...
def embedding_distance(tracks, detections, metric='cosine', use_last_feat=False):
    ...
    if use_last_feat:
        track_features = np.asarray([track.features[-1] for track in tracks], dtype=float)
    else:
